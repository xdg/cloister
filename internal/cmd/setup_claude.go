...
const (
	// AuthMethodExisting uses existing Claude login credentials.
	AuthMethodExisting AuthMethod = iota
...
This wizard helps you set up one of three authentication methods:

  1. Use existing Claude login (recommended)
     Extracts credentials from your existing Claude Code login.
...
// setupClaudeExtractor is the credential extractor used by setup claude.
// It can be overridden for testing.
var setupClaudeExtractor *claude.Extractor
...
// getSetupClaudeExtractor returns the credential extractor to use.
// Uses production implementation by default, but can be overridden for testing.
func getSetupClaudeExtractor() *claude.Extractor {
	if setupClaudeExtractor != nil {
		return setupClaudeExtractor
	}
	return claude.NewExtractor()
}
...
func runSetupClaude(cmd *cobra.Command, args []string) error {
	...
	switch authMethod {
	case AuthMethodExisting:
		if err := handleExistingLogin(cmd); err != nil {
			return err
		}
	case AuthMethodToken:
...
func saveCredentialsToConfig(cmd *cobra.Command, creds credentialResult, skipPerms bool) error {
	...
	case AuthMethodExisting:
		// No additional fields needed - credentials extracted at container start
	case AuthMethodToken:
...
// handleExistingLogin extracts credentials from the existing Claude login.
// On macOS, reads from the system Keychain.
// On Linux, verifies ~/.claude/.credentials.json exists.
func handleExistingLogin(cmd *cobra.Command) error {
	extractor := getSetupClaudeExtractor()

	creds, err := extractor.Extract()
	if err != nil {
		if errors.Is(err, claude.ErrCredentialsNotFound) {
			fmt.Fprintln(cmd.OutOrStdout(), "\nCredentials not found.")
			fmt.Fprintln(cmd.OutOrStdout(), "Please run `claude login` first to authenticate with Claude Code.")
			return err
		}
		return fmt.Errorf("failed to extract credentials: %w", err)
	}

	// Report success based on platform
	switch creds.Platform {
	case "darwin":
		fmt.Fprintln(cmd.OutOrStdout(), "\nSuccessfully extracted credentials from macOS Keychain.")
	case "linux":
		fmt.Fprintf(cmd.OutOrStdout(), "\nFound credentials file at: %s\n", creds.FilePath)
	}
